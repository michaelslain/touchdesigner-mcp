...
For more information on writing a shader, see the GLSL Category.
...
Sequence of TOP samplers

#### Name `sampler0name`

This is the sampler name that the GLSL program will use to sample from this TOP. The samplers need to be declared at the same dimensions as the TOP (sampler2D for a 2D TOP, sampler3D for 3D TOP).

#### TOP `sampler0top`

This is the TOP that will be referenced by the above sampler name above it. Exposed by the + Button, texture sampling parameters: Refer to the Texture Sampling Parameters article for more information on the parameters exposed by pressing the + button. The parameter prefix for each of the parameters is sampler[digit].
...
Sequence of named vectors to be used as uniforms in the shader
...
Extra Information for the GLSL COMP can be accessed via an Info CHOP.
