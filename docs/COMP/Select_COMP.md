...
The Select component selects a Panel Component from any other location. This allows a panel to appear in multiple other panels. Note that the Select component does not support child panels ie. through its output connector or embedded panels.
...
The Panel Component which is being selected.

#### Match Size `matchsize`

Set the Select COMP's width and Height to the referenced Panel Component's dimenions.

#### Follow Selection `followselection`

In the case of referencing a Select COMP, Follow Selection will retrieve the dimensions from the upper most component that is not a Select COMP.
...
Extra Information for the Select COMP can be accessed via an Info CHOP.
