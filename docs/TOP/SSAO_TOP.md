...
Determines the visual quality of the results. The higher the quality, the more computationally expensive it is.
...
For each pixel, rays are sent out in random directions to see if there is a surface nearby that would occlude the ambient light to this pixel. This parameter controls how many rays are sent out per pixel.

#### Sample Steps `samplesteps`

Since this is a 2D pixel-based operation, the rays need to sample multiple neighboring pixels along its path to check for points of interest. This parameter controls how many of these samples each ray does.

#### Surface Avoid Angle `surfaceavoid`

This parameter biases the angle of the rays so they don't travel too close to the plane that the starting point lays on (as determined by the point's normal).

#### SSAO Pass Resolution `ssaopassres`

The SSAO pass can be set to Full, Half, or Quarter the resolution of the input image.
...
The distance it searches from the current point for occluders (in object space units).

#### Contrast `contrast`

Controls the contrast of the SSAO contribution.

#### Attenuation `attenuation`

Controls the attenuation of the ambient lighting.

#### Edge Threshold `edgethresh`

To avoid the ambient occlusion from bleeding across the edges of objects onto other objects, an edge detect is done using the depth buffer to figure where one object ends and the next one begins. This parameter controls how sensitive the edge detection is.
...
Controls the sharpness of the blur operation.

#### Combine with Color `combinewithcolor`

By default the final ambient occlusion result will be multiplied by the color output of the Render TOP. You can just output the ambient occlusion results by turning this parameter off.
...
- Input 0: - Must be either a Render TOP or a Render Pass TOP.

## Info CHOP Channels

Extra Information for the SSAO TOP can be accessed via an Info CHOP.
