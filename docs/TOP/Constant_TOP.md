...
Specify the color values using different ranges. Note that this is only for purposes of setting the color, the data values in the TOP always use the RGBA Unit range 0-1.
...
Specify the output texture type to generate. Note: This parameter can only be used if there are no inputs to the TOP.
...
Sets the depth of the texture if the 'Output Type' parameter selects 3D Texture or 2D Texture Array.
...
Extra Information for the Constant TOP can be accessed via an Info CHOP.
