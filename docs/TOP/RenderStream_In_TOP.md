...
This node brings in image data sent over the RenderStream protocol from Disguise. The actual setup and sync of the RenderStream connection is done with the RenderStream In CHOP.
...
Controls if this node is pulling image data from RenderStream.

#### Name `name`

The name of the image to pull from RenderStream.
...
Extra Information for the RenderStream In TOP can be accessed via an Info CHOP.
