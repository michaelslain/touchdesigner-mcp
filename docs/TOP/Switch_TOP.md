...
Selects which input to pass though to the output. The first input is 0.

#### Blend between Inputs `blend`

Blends the input images when using floating point numbers in the Input parameter.
...
Extra Information for the Switch TOP can be accessed via an Info CHOP.
