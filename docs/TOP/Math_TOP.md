...
A menu of unary operations that are performed on each channel as it comes in to the Math TOP include:
...
A choice of operations is performed between the channels of the input TOP. Input and output channels are selected by the 'Combine Channels Input' and 'Combine Channels Output' parameters below. The Nth pixel of one channel is combined with the Nth pixel of other channels.
...
The resulting values can be converted to integers.
...
Select which channels are included in the input.

#### Combine Channels Output `outputchannels`

Select which channels are included in the output result.

### Multi-Add Page

#### Pre-Add `preoff`

First, add this value to each pixel of each channel.
...
- **fromranger1** `fromranger1`
- **fromranger2** `fromranger2`
...
- **fromrangeg1** `fromrangeg1`
- **fromrangeg2** `fromrangeg2`
...
- **fromrangeb1** `fromrangeb1`
- **fromrangeb2** `fromrangeb2`
...
- **fromrangea1** `fromrangea1`
- **fromrangea2** `fromrangea2`
...
Extra Information for the Math TOP can be accessed via an Info CHOP.
