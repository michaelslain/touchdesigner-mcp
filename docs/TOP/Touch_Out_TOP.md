...
It can output Hap Q, Hap Q Alpha, and Uncompressed video.
...
Which TCP/IP port to send the images on.

#### Active `active`

Sends image data over the TCP/IP connection when active is on.

#### FPS `fps`

This specifies how many frames pass before sending another frame. This effectively lowers frame rate and bandwidth of the output. For example, if the input frame rate is 60 and Send Step Size is 2, the output frame rate will be 30.

#### Video Codec `videocodec`

Choose between Uncompressed and HAP Q codecs to transmit the image stream.
...
Forces the TOP to cook every frame.
...
Extra Information for the Touch Out TOP can be accessed via an Info CHOP.
