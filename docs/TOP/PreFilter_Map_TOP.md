...
The PreFilter TOP uses spherical harmonics to calculate a Pre-Filtered Diffuse Map or Pre-Filtered Specular Map for use in the Environment Light COMP. The output of the PreFilter Map TOP can not have further processing done to them before being used by the Environment Light.
...
Select between calculating the PreFilter for the Diffuse Map or Specular Map.
...
Extra Information for the PreFilter Map TOP can be accessed via an Info CHOP.
