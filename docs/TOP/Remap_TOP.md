...
The selected input's width-height resolution will be used for the final resolution unless manually overwritten on the Common Page.

- **Input 1** `input1`
- **Input2** `input2`

#### Depth Res Source `depthressource`

The selected input's depth will be used for the final depth.
...
Select which of the second input's channels to use for the horizontal warp of the first input.
...
Select which of the second input's channels to use for the vertical warp of the first input.
...
Select which of the second input's channels to use for the z warp of the first input.
...
Reverses the direction of the horizontal warp. A pixel value of 0 for the horizontal warp will mean that it fetches a pixel from the right most column of the first input.

#### Flip Vertical Direction `flipvert`

Reverses the direction of the vertical warp. A pixel value of 0 for the vertical warp will mean that it fetches a pixel from the top most row of the first input.

#### Flip Z Direction `flipz`

Reverses the direction of the z warp. A pixel value of 0 for the z warp will mean that it fetches a pixel from the back most slice of the first input.

#### Extend `extend`

Controls what is returned from your first input when the second input's values are outside the [0-1] range.
...
Select which interpolation method to apply when sampling the first input.
...
Extra Information for the Remap TOP can be accessed via an Info CHOP.
