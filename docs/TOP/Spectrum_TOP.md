...
Selects the transform mode (ie. direction). Either DFT or IDFT.
...
Selects the coordinate system for output/input in the DFT/IDFT cases respectively.
...
Selects which channel of the input TOP to use for the discrete fourier transform.
...
When enabled, will perform the DFT/IDFT by row.
...
Extra Information for the Spectrum TOP can be accessed via an Info CHOP.
