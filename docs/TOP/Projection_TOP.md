...
At this time the only option is a Cube Map input. You can generate a Cube Map by rendering one using the Render TOP, or converting 2D images into one using the Cube Map TOP.
...
Select output format from the menu.

- **Cube Map** `cubemap`
- **Equirectangular** `equirectangular` - A 2D Texture is usually 2:1 aspect ration. This format is typically used to map the texture onto a sphere with UVs set to to lat/long. This format is very popular for 360 video. A more details explanation is available here.
...
Only enabled in Fish-Eye output mode to specify the field of view of the Fish-Eye camera.
...
Extra Information for the Projection TOP can be accessed via an Info CHOP.
