...
The MPCDI TOP lets you load calibration data stored in the MPCDI format. Please refer to the MPCDI entry for a complete guide on TouchDesigners integration with the MPCDI Standard.
...
#### Output by `outputformat`

The menu determines how the output regions are laid out, in single or grid format.
...
Determines the maximum rows and columns when output by grid is selected.
...
#### Alpha Beta Blending `alphabeta`

Toggle to apply alpha-beta blending.

#### Gamma Encoding `gamma`

Toggle to apply gamma encoding.
...
Extra Information for the MPCDI TOP can be accessed via an Info CHOP.
