...
In addition to all the inputs attached, you can specify more using the TOPs listed in this field. Example: ramp* will composite all TOPs whose name starts with ramp.

#### Preview Grid `previewgrid`

This outputs an image showing the effect of all operation types in a grid, with the inputs swapped on the right side of each tile.

#### Select Input `selectinput`

Instead of doing the composites, this causes only one of the inputs to pass through.

#### Input Index `inputindex`

When passing through an input with Select Input on, this is the index of the image that is passed through.
...
Swaps the order of the input pairs. A operation B is changed to B operation A. Operations like Add don't matter, but many do, like Over and Hard Light.

### Transform Page

#### Fixed Layer `size`

The selected input will become the fixed layer and the other input will be the overlay. This does not change the order of the composite (Input1 + Input2), only which layer is considered fixed and which layer is adjustable by the parameters on the Transform page. The resolution and aspect ratio of the Fixed Layer is used as the composite's final resolution and aspect ratio unless manually on the Common Page
...
Determines how the Overlay layer (Overlay layer is the input that is NOT the Fixed Layer) fills the composite.
...
Specify the horizontal alignment of the Overlay.
...
Specify the vertical alignment of the Overlay.
...
Sets the extend (or repeat) conditions of the Overlay layer. This parameter determines what happens at the edges of the Overlay layer.
...
Rotates the Overlay layer. Increasing values rotate clockwise, decreasing values rotate counter-clockwise.
...
Sets the units used in the Translate parameter.
...
Sets the units used in the Pivot parameter.

#### Translate Step `tstep`

Translates the Overlay layers cumulatively, starting at the 3rd input (index 2). That layer is translated by the amount specified, the next layer is translated by 2 times that amount, the following layer by 3 times that amount and so on.
...
Sets the units used in the Translate Step parameter

#### Legacy Transform `legacyxform`

When enabled, will use the legacy method of building the transform matrix, which has inverted rotation and transform order.
