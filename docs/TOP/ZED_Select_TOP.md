...
This TOP is used to select extra images from the device configured in the ZED TOP. Such as the depth, color image, disparity map etc.
...
Controls whether this TOP is retrieving image data from the device. The primary ZED TOP must also be active to receive data.

#### ZED TOP `zedtop`

The name of the primary ZED TOP that is configuring the camera. The primary TOP controls which camera the select TOP receives data from.
...
Selects between the Color, Depth, Confidence, Disparity, Normals, Point Cloud or Spatial Texture modes.
