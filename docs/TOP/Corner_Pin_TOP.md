...
- **extractp31** `extractp31`
- **extractp32** `extractp32`
...
- **extractp41** `extractp41`
- **extractp42** `extractp42`
...
- **extractp11** `extractp11`
- **extractp12** `extractp12`
...
- **extractp21** `extractp21`
- **extractp22** `extractp22`

#### Top Right Unit `extractp2unit`

#### Extend `extend`

Determines how the image is handled at its edges.
...
Pick between Bilinear or Perspective Extraction.
...
To perform the Extract and Pin operations, the image is placed on a polygonal grid to break it up into blocks that can be distorted. Grid Refinement sets the number of divisions in this grid.

#### Background Color `bgcolor`

Color applied behind the foreground image. The background is visible when the corners of the Corner Pin have been positioned inside the original image space. Set the Bottom Left x-position to 1.0 and you will see the background color in the bottom left corner of the image.
...
Extra Information for the Corner Pin TOP can be accessed via an Info CHOP.
