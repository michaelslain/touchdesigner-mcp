...
The wrapping method used when applying limits to the pixel values in the image.
...
The minimum value that any of the channels in the output image can have.

#### Maximum Value `max`

The maximum value that any of the channels in the output image can have.

#### Positive Only `positive`

Apply an absolute value function after all other limits and quantizations are calculated e.g. all negatives are made positive.

#### Normalize `norm`

Normalize values in the output image so that they are all scaled and shifted to fall between the Normalized Minimum and Maximum (0 to 1 by default). This operation requires multiple internal render passes and is slightly slower than other limit operations. Normalization is done after all other limits and quantizations are applied.

#### Normalize Minimum `normmin`

The minimum value for pixels after normalization.

#### Normalize Maximum `normmax`

The maximum value for pixels after normalization.

### Quantize Page

#### Quantize Value `quantvalue`

The function used to quantize the pixel values in the output image.
...
The quantization step size for pixel values.

#### Value Offset `voffset`

An offset for the quantization step so that it doesn't have to lie on zero.

#### Quantize Position `quantpos`

The function used for spacial quantization e.g. quantizing the UV coordinates so that pixel values are merged into larger blocks.
...
The size of the spacial quantization step in UV space (0-1)

#### Position Offset `posoffset`

An offset applied to the spacial quantization so that the steps do not have to start at 0,0 (measured in 0-1 UV space).
...
Extra Information for the Limit TOP can be accessed via an Info CHOP.
