...
Send image data out to RenderStream. The actual setup and sync of the RenderStream connection is done with the RenderStream In CHOP.
...
Controls if this node sends data out to RenderStream.

#### Stream Index `streamindex`

The stream index to send to.

#### Profile Output CHOP `profilechop`

A CHOP that contains channels of information that will be sent along with the image to be shown for profiling information in the D3 user interface.
...
Extra Information for the RenderStream Out TOP can be accessed via an Info CHOP.
